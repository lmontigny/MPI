...
int main(int argc, char *argv[]){
	...
	if ( argc != 2 ){
    	printf("Usage: mpiexec -n nodecount ring1ba n \n\n");
		return 1;
    }
	...
	for ( i = 0 ; i < REPETITIONS ; ++i ){
		...
		// send to left
		MPI_Send(&a,n,T_mpisendtype,leftid,0, MPI_COMM_WORLD); 
		// receive from the right
		MPI_Recv(&recv,n, T_mpisendtype, rightid, 0, MPI_COMM_WORLD, &recv_status);
		Sum(recv, b, n);

		// send to the right
		MPI_Send(&a,n,T_mpisendtype,rightid,0, MPI_COMM_WORLD); 
		// receive from the right
		MPI_Recv(&recv,n, T_mpisendtype, leftid, 0, MPI_COMM_WORLD, &recv_status);
		Sum(recv, b, n);
		Sum(a, b, n);
